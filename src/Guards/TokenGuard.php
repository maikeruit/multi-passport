...
namespace Sumatoreo\MultiPassport\Guards;
...
class TokenGuard
{
    ...
    public function user(Request $request)
    {
        session()
            ->put(
                ...
                $request->input(
                    'guard',
                    config('multipassport.passport.guard')
                )
            );
    ...
    /**
     * Подстановка модели при множественнй авторизации через разные таблицы
     */
    protected function setModelViaGuard() {
        $guard = session('guard');
     ...
     * @param $psr
     */
    protected function setSessionGuard($psr) {
        $guard = DB::table('oauth_access_tokens')
            ->where('id', $psr->getAttribute('oauth_access_token_id'))
            ->first()
            ->guard;
     ...
     * @return bool
     */
    protected function checkMiddleware(Request $request) {
        $middleware = $request->route()->middleware();
        $middlewareGuard = 'auth:' . session('guard');

        return in_array($middlewareGuard, $middleware);
    }
    ...
    protected function authenticateViaBearerToken($request)
    {
        ...
        $this->setSessionGuard($psr);
        $this->setModelViaGuard();

        if (!$this->checkMiddleware($request)) {
            return;
        }
